...
from ibm.teal.registry import SERVICE_DB_INTERFACE, get_service,\
    ...
    SERVICE_EVENT_Q, get_logger, SERVICE_EVENT_MONITOR
    ...
    def start(self):
        ...
        while self.running:
            ...
            if ctl == MONITOR_CONTROL_WAIT_ON_NOTIFY:
                self.notifier.wait()
            elif ctl == MONITOR_CONTROL_PROCESS_ONE_EVENT:
                # Process just one of the events (but do the full query)
...
class TestEventAnalyzerEventCapture(EventAnalyzer):
    ...
    def base_analyze_event_CHECKPOINT(self, event):
        ...
        if self.last_rec_id % 2 == 0:  # only checkpoint if even 
...
class TestEventAnalyzerDelay(EventAnalyzer):
    ...
    def will_analyze_event(self, event):
        ...
        #print event.event_id in self.analyze_event_ids
...
class BaseTestCheckpoint(TealTestCase):
    ...
    def _check_checkpoint(self, rows, expected_dict, noisy=True, only_names=None):
    ...
    def _wait_for_checkpoint(self, expected_dict, in_tries=None, dbi=None, only_names=None):
        if in_tries is None:
            tries = 9
        else:
            tries = in_tries
        ...
        while tries > 0:
            ...
            try:
                self._check_checkpoint(ckp_rows, expected_dict, noisy=False, only_names=only_names)
                match = True
                break
            except:
                pass
            ...
            tries -= 1 
        if match == False:
            self._check_checkpoint(ckp_rows, expected_dict, noisy=True, only_names=only_names)
